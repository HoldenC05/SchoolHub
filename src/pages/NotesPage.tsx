...
function NotesPage({
      ...
      <div key={note.id}>
        <div
          ...
          className={`group flex cursor-grab items-center gap-1 rounded-md px-2 py-1 ${
            isSel ? "bg-indigo-500/15 text-indigo-200" : "text-slate-400 hover:bg-slate-800/70 hover:text-slate-200"
          }`}
  ...
  return (
    <div className="flex h-full gap-4">
      <aside className="flex w-60 shrink-0 flex-col overflow-hidden rounded-xl border border-slate-800 bg-slate-900/60">
        <div className="flex items-center justify-between border-b border-slate-800 px-3 py-2">
          ...
          <button
            ...
            className="rounded-md px-2 py-1 text-sm font-medium text-slate-300 transition-colors hover:bg-slate-800"
            title="New note"
          >
            + New note
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-1.5">
          {loading ? (
            ...
            <div className="px-2 py-6 text-center">
              ...
              <p className="mt-1 text-xs text-slate-600">Create your first one above</p>
            </div>
          ) : (
            <div
              onDragOver={(e) => e.preventDefault()}
            ...
            >
              ...
              <p className="mt-2 border-t border-dashed border-slate-800 px-2 pt-2 text-[10px] text-slate-600">
      ...
      <main className="flex-1 overflow-hidden rounded-xl border border-slate-800 bg-slate-900/60">
        {selectedNote ? (
          <NoteEditor
            key={selectedNote.id}
            note={selectedNote}
