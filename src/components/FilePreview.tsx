...
function markdownDoc(md: string): string {
  return `<!doctype html><html><head><meta charset="utf-8"><style>body{background:#f8fafc;color:#334155;font-family:ui-sans-serif,system-ui,sans-serif;padding:28px;line-height:1.7;max-width:820px;margin:0 auto}h1,h2,h3{color:#0f172a}code{background:#f1f5f9;padding:2px 6px;border-radius:4px;color:#6d28d9}pre{background:#f8fafc;padding:12px;border-radius:8px;overflow-x:auto}pre code{background:none;padding:0}a{color:#4f46e5}img{max-width:100%}blockquote{border-left:3px solid #6366f1;padding-left:12px;color:#475569;margin-left:0}</style></head><body>${md}</body></html>`;
}
...
export function FilePreview({
  ...
  return (
    ...
    >
      <div
        className="flex h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl animate-modal-in"
      ...
      >
        ...
        <div className="flex-1 overflow-hidden bg-slate-50">
          {loading ? (
            ...
            <pre className="h-full w-full overflow-auto whitespace-pre-wrap p-4 font-mono text-xs text-slate-700">
              {text}
            </pre>
          )}
        </div>
